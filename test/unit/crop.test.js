...
import { collectTiles, createCroppedImage } from '../../src/crop'

import knpGeoJSON from '../resources/knp_boundary.js'
...
const X21Y09 = fs.readFileSync(path.join(__dirname, '..', '/resources/X21Y09.png'))
const X21Y10 = fs.readFileSync(path.join(__dirname, '..', '/resources/X21Y10.png'))
...
describe('crop', () => {
  ...
  //     ]]
  //   }
  //   const buffer = await createCroppedPNGWithJimp(collected, geoJSONGeometry)
  ...
  it('can create an output image 1', async () => {
    const collected = await collectTiles([
      { bbox: new Box2(new V2(100, 50), new V2(200, 150)), bytes: testPNG100x100 },
      { bbox: new Box2(new V2(200, 50), new V2(300, 150)), bytes: testPNG100x100 }
    ])
    const geoJSONGeometry = {
      type: 'Polygon',
      coordinates: [[
    ...
    const img = await createCroppedImage(collected, geoJSONGeometry)
    const outputPath = path.join(__dirname, '..', 'resources', 'output1b.png')
    img.write(outputPath)
  })

  it('can do a KNP example', async () => {
    const collected = await collectTiles([
      { bbox: new Box2(new V2(30, -25), new V2(40, -15)), bytes: X21Y09 },
      { bbox: new Box2(new V2(30, -35), new V2(40, -25)), bytes: X21Y10 }
    ])
    const geoJSONGeometry = knpGeoJSON.features[0].geometry
    const img = await createCroppedImage(collected, geoJSONGeometry)
    const outputPath = path.join(__dirname, '..', 'resources', 'knp.png')
    img.write(outputPath)
  }).timeout(20000)
})
